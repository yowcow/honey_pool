...
-export([get/1, get/2, get/3, get/4, post/2, post/3, post/4, post/5, request/6,
         return_to/3, dump_state/0, summarize_state/0]).
...
-type resp() ::
  {ok, {status(), resp_headers(), binary() | no_data}} | {error, honey_pool_error()}.
...
handle_request_result(Result, ReturnTo, HostInfo, {Pid, _} = Conn, Method, Url) ->
  case Result of
    ...
    {error, {timeout_error, _}} = TimeoutErr ->
...
-spec handle_ok_response_body(pid(),
                              gun:stream_ref(),
                              reference(),
                              resp_headers(),
                              timeout()) ->
...
checkout(HostInfo, Timeout0) ->
  {Elapsed, Result} =
    timer:tc(fun wpool:call/4, [?WORKER, {checkout, HostInfo}, best_worker, Timeout0]),
  try Result of
...
summarize_state(#{await_up_conns := AC,
                  checked_in_conns := IC,
